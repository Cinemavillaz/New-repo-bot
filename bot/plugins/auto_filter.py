...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"⭐️ 𝚃𝚘𝚃𝚊𝙻 𝙿𝚊𝙶𝚎𝚂 1/{len_result if len_result < max_pages else max_pages} 🔶", callback_data="ignore")
        ])
        ...
        try:
            await bot.send_message(
                ...
                caption=f" 𝘾𝙡𝙞𝙘𝙠 𝙔𝙤𝙪𝙧 𝙁𝙞𝙡𝙚 📁 𝙎𝙏𝘼𝙍𝙏  𝙏𝙝𝙚 𝘽𝙤𝙩  𝐑𝐞𝐬𝐮𝐥𝐭𝐬 𝐅𝐨𝐮𝐧𝐝:- {(len_results)} 𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐞𝐝 𝐌𝐨𝐯𝐢𝐞 : <code>{query}</code>",
